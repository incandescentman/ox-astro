...
For the cross-repo publishing map, start here:
=~/Dropbox/github/docs/org-roam-web-publishing-system.org=

For destination nickname mapping, see:
=~/Dropbox/github/ox-astro/docs/destinations.org=
...
To use this exporter, save all ~ox-astro-*.el~ files to your Emacs load path, then add the following to your Emacs configuration (~init.el~):
...
| ~ox-astro-helpers.el~         | Loader for helper modules                            |
| ~ox-astro-helpers-core.el~    | Core utilities (slugify, date formatting, etc.)      |
| ~ox-astro-export-helpers.el~  | Export utilities and file management                 |
| ~ox-astro-transcode.el~       | Element transcoding (headings, links, blocks)        |
| ~ox-astro-metadata.el~        | Front matter extraction and generation               |
| ~ox-astro-handlers.el~        | Export filters and content processing pipeline       |
| ~ox-astro-table-handlers.el~  | Table → Markdown conversion                          |
| ~ox-astro-image-handlers.el~  | Image detection, copying, and ~<Image>~ component output |
...
- **org-roam Integration**: Fully compatible with org-roam files—property drawers are stripped from body content (IDs go to frontmatter as ~orgRoamId~), and template boilerplate (~- **Links:**~, ~- **Source:**~) is automatically removed.
...
- **Date-Only Title Guard**: If the file name or =#+TITLE= is just a date, the first heading becomes the title/slug/output filename; later non-date titles override date placeholders.
...
2.  Set the destination folder using the `#+DESTINATION_FOLDER`, `#+DESTINATION-FOLDER`, or `#+DESTINATION` keyword. You can use a nickname from your `org-astro-known-posts-folders` list or provide a full, absolute path to any directory.
...
For the shared multi-repo metadata standard used by ~astro-roam~, ~life-web~, and ~parents-web~, prefer keeping canonical metadata in the Org source file headers.
...
| ~#+THEME~              | ~theme~       | Visual theme for the post (e.g., ~claude~, ~chatgpt~, ~gemini~). See Theme Sections below. |
| ~#+DESTINATION_FOLDER~ (aliases: ~#+DESTINATION-FOLDER~, ~#+DESTINATION~) | (none)      | Specifies the export destination folder.        |
...
- Sources: Use ~#+ASTRO_STORY_TYPE~ or ~#+STORY_TYPE~. The ~ASTRO_*~ variant takes precedence.
- Validation: ~STORY_TYPE~ is constrained. The exporter currently accepts:
  - ~full-story~
  - ~vignette~
  - ~snapshot~
  - ~fragment~
  - ~conversation~
- Practical rule: If you want labels like ~interview~ or ~profile~, keep those in ~#+TAGS~ rather than forcing them into ~#+STORY_TYPE~.

Example:

#+begin_src org
#+STORY_TYPE: conversation
#+TAGS: interview, dad, childhood
#+end_src
...
Gallery blocks (`#+BEGIN_GALLERY`) automatically render using the shared `ImageGallery` component exposed by `@jaydixit/astro-utils/components/ImageGallery.astro`, keeping every Astro app on the same implementation.
...
~ox-astro~ supports multi-AI blog posts where different sections can have distinct visual themes (e.g., ChatGPT, Gemini, Claude). This is useful for posts that showcase conversations with multiple AI models.

** Keywords

| Keyword      | Purpose                                                        |
|--------------+----------------------------------------------------------------|
| ~#+THEME:~   | Sets the visual theme for a section (~chatgpt~, ~gemini~, ~claude~, ~default/jay/author/human~) |
| ~#+MODEL:~   | Displays a model banner with the AI name (e.g., "Claude Sonnet 4.5") |

** Placement Rules

The behavior of ~#+THEME:~ and ~#+MODEL:~ depends on where they appear in your Org file:

*** Document Level (before any heading)
- ~#+THEME:~ → Goes into YAML frontmatter only (~theme: claude~)
- ~#+MODEL:~ → Emits a model banner once near the top of the body
- Use this for single-AI posts where the entire page uses one theme

*** Body Level (after a heading)
- ~#+THEME:~ → Emits inline JSX marker: ~{/* theme: claude */}~
- ~#+MODEL:~ → Emits visible banner: ~<div class="model-banner">Claude Sonnet 4.5</div>~
- Use this for multi-AI posts or to show model attribution

** Examples

*** Single-AI Post (page-wide theme)
#+begin_src org
...
Result: Frontmatter includes ~theme: claude~, entire page styled with Claude theme.

*** Multi-AI Post (themed sections)
#+begin_src org
#+TITLE: AI Comparison
#+THEME: chatgpt
...
Result: Each section wrapped in themed div with model banner showing the AI name and logo.

*** Resetting to Default Styling
Use ~#+THEME: default~, ~#+THEME: jay~, ~#+THEME: author~, or ~#+THEME: human~
to return to unthemed (regular blog / author commentary) sections:
...
This section uses the site's default styling, not any AI theme.
#+end_src

** CSS Requirements

Your Astro site needs corresponding CSS for each theme:
- ~.chatgpt-theme-section~ and ~.chatgpt-theme-wrapper~
- ~.gemini-theme-section~ and ~.gemini-theme-wrapper~
- ~.claude-theme-section~ and ~.claude-theme-wrapper~

Each theme CSS should include ~.model-banner~ styling with the appropriate logo icon.
