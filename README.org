...
- Destination path: `public/pdfs/{slug}/filename.pdf` (slug derived from the post)
- Org source update: the original absolute path is replaced with the new local file path in `public/pdfs/...`
- MDX output: links are written as site-relative `/pdfs/{slug}/filename.pdf` with any spaces encoded to `%20`.
