...
- Validation: ~STORY_TYPE~ is constrained. The exporter currently accepts:
  ...
  - ~interview~
  - ~profile~
- Practical rule: Use ~#+STORY_TYPE~ for content shape values like ~interview~, ~profile~, or ~conversation~. Use ~#+TAGS~ for topical labels like ~dad~, ~childhood~, or ~physics~.
