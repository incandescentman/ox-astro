...
** Summary
The exporter now maps org-roam metadata (`DATE_OCCURRED`, `ERA`, `PLACES`, `THEMES`, `STORY_TYPE` and their `ASTRO_*` variants) into MDX front matter, and the parsing logic lives in a dedicated `ox-astro-metadata.el` helper module.
...
** Key Behavior
- `dateOccurred`, `era`, `places`, `themes`, and `storyType` are emitted in front matter when present.
- Both plain and `ASTRO_` keyword forms are supported for each new attribute.
- List-style keywords respect quoted multi-word tokens (e.g., `"New Haven"`).
