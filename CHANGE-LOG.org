...
The ox-astro exporter only supported posts folder selection through predefined nicknames in =org-astro-known-posts-folders=. Users could not specify arbitrary absolute paths directly in the =#+POSTS_FOLDER= keyword.
...
Users can now specify any valid absolute directory path in =#+POSTS_FOLDER= without needing to pre-configure it in their Emacs settings. This enables:
