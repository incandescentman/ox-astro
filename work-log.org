* 2026-02-12 - Studies frontmatter mode + socratic-studies destination

** Summary
Added a studies export mode so academic references can live in a dedicated content collection (one file per study).

** Changes
- Added =socratic-studies= destination mapping in =org-astro-known-posts-folders=
- New frontmatter mode (=studies=) emits study fields: =id=, =title=, =authors=, =year=, =href=, =abstract=, =tags=
- Optional fields supported: =doi=, =journal=, =studyType=, =sampleSize=, =method=, =keyFindings=, =limitations=, =evidence=, =pdf=
...
** Also Fixed: Double ! in Markdown Images
Org inline image syntax ~![[path][desc]]~ was producing ~!![alt](path)~ in MDX:
- The ~!~ from org's inline image prefix was preserved as text
- Combined with ~![alt](path)~ from the ~:jsx~ value = double ~!!~

Fixed by adding cleanup in ~org-astro-final-output-filter~ to normalize ~!![~ to ~![~
before processing images (ox-astro-handlers.el line 623).
...
- Markdown images render correctly without double ~!~
...
* 2026-03-17 - Reset theme aliases for author commentary

** Summary
- Normalized `#+THEME:` reset aliases so `default`, `jay`, `author`, and `human` all export as the canonical `default` reset.
- Applied the normalization consistently to inline theme markers, doc-level frontmatter, and unique theme counting.
- Added regression coverage for inline `human` and doc-level `jay` usage.

** Impact
- Author-facing reset themes now read more naturally in Org files without changing downstream Astro behavior.
- Frontmatter and inline markers stay canonical, so the rendering pipeline remains predictable.
