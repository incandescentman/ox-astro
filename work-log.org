...
- Restored narrowed-buffer exports: if the buffer is narrowed, export treats it as a subtree in both passes and stays narrowed afterward.
...
* 2025-11-23 (later)
** Session Summary
- Preserved user narrowing after Astro export by reapplying the narrowed region bounds post-export while still running both passes as subtree exports.
- Verified all regression tests continue to pass after the narrowing fix.

** Handoff

*COMPLETED (Safe to rely on)*
- [x] Narrowing restored after export without affecting subtree export behavior: `ox-astro.el`
- [x] Tests re-run: `test/date-title-and-subtree-test.el`

*IN-PROGRESS (Requires completion)*
- [ ] None

*CONTEXT FOR NEXT SESSION*
- Files touched: `ox-astro.el`, `work-log.org`
- Known unknowns: None
