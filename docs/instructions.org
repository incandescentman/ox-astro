...
- *Template boilerplate* is removed from output:
  ...
  - Converted preamble link lines before the first heading (e.g., =[📄 Note](file:///...)=)
...
| Bold list item (content)   | =- **Links:** [[id:...]]= | Stripped when part of org-roam preamble |
| Standalone label           | =Links:=                 | Stripped |
| Converted markdown links   | =[📄 Note](file:///...)= | Stripped in pre-heading preamble |
| Property drawer lines      | =:ID: abc123=            | Stripped |

Note: The exporter strips these patterns specifically in the top pre-heading preamble area used by org-roam templates.

* Debug and Development Options

** Reloading ox-astro without restarting Emacs
If you iterate on ox-astro code, prefer a clean reload to avoid stale function
definitions. Repo-local =.elc= files are ignored by Git but still affect
runtime loading, so the reload must delete them before requiring modules. This
snippet unloads modules, removes bytecode, and reloads from the repo:

#+begin_src emacs-lisp
(defconst ox-astro-repo "/Users/jay/Dropbox/github/ox-astro")

(defun ox-astro-delete-bytecode ()
  "Delete repo-local ox-astro bytecode so Emacs loads source files."
  (when (file-directory-p ox-astro-repo)
    (dolist (file (directory-files-recursively ox-astro-repo "\\.elc\\'"))
      (when (file-regular-p file)
        (delete-file file)))))

(defun ox-astro-reload ()
  "Reload ox-astro and its modules."
  (interactive)
  (let ((repo ox-astro-repo))
    (add-to-list 'load-path repo)
    (setq load-prefer-newer t)
    (ox-astro-delete-bytecode)
    (dolist (feat '(ox-astro-table-handlers
                    ox-astro-image-handlers
                    ox-astro-pdf-handlers
                    ox-astro-handlers
                    ox-astro-helpers
                    ox-astro-transcode
                    ox-astro-export-helpers
                    ox-astro-helpers-core
                    ox-astro-metadata
                    ox-astro-config
                    ox-astro))
      (when (featurep feat)
        (ignore-errors (unload-feature feat t))))
    (load-file (expand-file-name "ox-astro.el" repo))
    (dolist (feat '(ox-astro-config
                    ox-astro-helpers-core
                    ox-astro-export-helpers
                    ox-astro-transcode
                    ox-astro-helpers
                    ...
                    ox-astro-image-handlers
                    ox-astro-pdf-handlers
                    ox-astro-table-handlers
                    ox-astro-handlers))
      (require feat nil t))
    (message "ox-astro reloaded successfully")))
#+end_src

If Emacs logs this during export/load:
- =Source file '.../ox-astro-*.el' newer than byte-compiled file; using older file=

then your runtime may still be using stale bytecode. In that case:
- Re-run =ox-astro-reload=
- Check =(symbol-file 'org-astro-src-block)=; it should resolve to
  [[file+emacs:/Users/jay/Dropbox/github/ox-astro/ox-astro-transcode.el][📄 ox-astro-transcode.el]],
  not =ox-astro-transcode.elc=
- If behavior still does not change, remove stale =.elc= files in the repo and
  reload again
...
** Troubleshooting Deferred Org Values
If export fails with an error like:
- =wrong-type-argument stringp [org-element-deferred ...]=

This means Org returned a deferred value wrapper where ox-astro expected a
plain string (commonly in =src-block= or =example-block= content on newer Org
versions).

Checklist:
- Ensure you are running a version that includes =org-astro--resolve-deferred-string=
  and uses it in =org-astro-src-block= and =org-astro-example-block=.
- Reload ox-astro modules after updating (see =ox-astro-reload= above).
- Re-run export on the same file to confirm the deferred-value path is now handled.

** Troubleshooting Empty Conversation Blocks
If exported MDX shows empty conversation fences like:
- =```user=
- blank line
- =```=

or conversation fences disappear entirely, but the Org source has text in
=#+begin_src user= / =assistant= / =prompt= blocks:

Checklist:
- Confirm source blocks in Org are non-empty.
- Check Babel language defaults at runtime:
  - =org-babel-default-header-args:user=
  - =org-babel-default-header-args:assistant=
- If any conversation language resolves to =:exports results=, block content can be dropped because no-op executors return nil results.
- Ensure exporter path forces =:exports code= for conversation-style languages during export.
- Reload exporter modules and re-export after changes.

Quick verification:
- Count user fences and empty user fences in output; they should match total>0 and empty=0 for normal transcripts.
...
** Troubleshooting Leaked Links/Source Preamble
If exported MDX shows a top-of-body line like:
- =[📄 Some Note](file:///...)=

it means the org-roam preamble was rendered as plain markdown links and bypassed older
label-based cleanup patterns.

Checklist:
- Ensure you are running a version that strips converted preamble links before the first heading.
- Reload ox-astro modules after updating (see =ox-astro-reload= above).
- Re-export and verify there is no pre-heading =file:///...=.md link line.
...
#+begin_src org
#+HERO_IMAGE: /path/to/hero-image.png
#+HERO_IMAGE_ALT: A brief description
...
*** Display Behavior
- *On page*: Credit — and only credit — renders visibly, as a ~<figcaption>~ below the
  image (exact text you provide). ~IMAGE-CAPTION~ / ~IMAGE-PROMPT~ never appears on the
  page.
- *In lightbox*: Both credit (bold) and caption (italic) appear at the bottom (no
  automatic prefix). ~IMAGE-CAPTION~ is *only* ever visible here, after the reader
  clicks the image.
- Easy to confuse: if you want text under the image, use ~IMAGE-CREDIT~, not
  ~IMAGE-CAPTION~ — the keyword names do not match where the text shows up.
...
*** Notes
- IMAGE_* keywords must immediately follow the body image they describe (blank lines ok); HERO_* keywords should appear at document level
- Underscores and hyphens are interchangeable (e.g., ~IMAGE_CREDIT~ = ~IMAGE-CREDIT~)
- All hero images (explicit =HERO_IMAGE= / =COVER_IMAGE= / =ASTRO_IMAGE= and fallback first-inline-image) are suppressed from the body by default to avoid duplication with the layout hero; use =#+ATTR_ORG: :repeat t= to force an image to appear inline even when it is the hero
- If =HERO_IMAGE= is not set, ox-astro falls back to =COVER_IMAGE= / =ASTRO_IMAGE= and then the first inline image
- =HIDE_HERO_IMAGE= hides the page-level hero component; it does not force inline image suppression
- Public web paths (e.g., =/og/...= or =https://...=) are preserved as-is for hero/cover images
- If your Astro app resolves frontmatter images through a helper (e.g., =findImage()= + =import.meta.glob=), include all exported formats there (including =avif=) or hero images may render blank.
...
| =#+hero_image:=    | =image=         | Explicit hero image (highest priority) |
| =#+hero_image_alt:= | =imageAlt=     | Alt text for hero image            |
| =#+cover_image:=   | =image=         | Fallback hero image                |
| =#+astro_image:=   | =image=         | Alias for cover image              |
| =#+draft:=         | =draft=         | Boolean                            |
...
** Placement Rules
- *Document level* (before any heading): ~#+THEME:~ goes to frontmatter only; ~#+MODEL:~ emits a banner once near the top
...
- Author commentary: Use ~#+THEME: default~, ~jay~, ~author~, or ~human~ to reset styling
