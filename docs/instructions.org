...
definitions (especially when =.elc= files are present). This snippet unloads
modules, reloads from the repo, and verifies core helpers are present:

#+begin_src emacs-lisp
(defun ox-astro-reload ()
  "Reload ox-astro and its modules."
  (interactive)
  (let ((repo "/Users/jay/Dropbox/github/ox-astro"))
...
- If behavior still does not change, remove stale =.elc= files in the repo and reload again
