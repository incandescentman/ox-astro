...
- Supported mappings:
  ...
  - =#+places:= → `places` array
  - =#+themes:= → `themes` array
  - =#+story_type:= → `storyType` (validated against `full-story`, `vignette`, `snapshot`, `fragment`, `conversation`)
  - =#+filetags:= → merged into `tags` array (deduped with =#+TAGS:= and =#+ASTRO_TAGS:=)
