...
The specific duplicate banner bug for section-leading heading metadata is now
fixed.

- Real verification target:
  - Source:
...
- Verified output shape:
  ...
  - one =# Claude= heading
- Fix direction that worked:
  - pre-mark section-leading =#+THEME:= / =#+MODEL:= keywords as
    =:astro-consumed= in a parse-tree filter before transcoding
  - keep the headline handler as the sole emitter for hoisted theme/model
    prefixes

The analysis below remains useful historical context for older theme/model
failures, especially the distinction between document-level metadata and
heading-scoped metadata.
