...
Option A: Have ~org-astro-body-filter~ be the SOLE emitter of document-level
MODEL banners. Modify ~org-astro-keyword~ to skip MODEL when at document level:

#+begin_src elisp
((string-equal key "MODEL")
 (if (and body-level (not (org-astro--is-document-level-p keyword)))
     (format "<div class=\"model-banner\">%s</div>\n\n" value)
   ""))
#+end_src
...
** Proposed Fix
In post-processing, remove theme markers that are immediately followed by
another theme marker with no content between them. Or prevent emission
of consecutive identical/overridden theme markers.
