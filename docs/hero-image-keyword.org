...
3. If no =#+HERO_IMAGE:=, fall back to first inline image (also suppressed from body)
4. The hero image should NOT be duplicated in the body content
5. Use =#+ATTR_ORG: :repeat t= before an image to force it inline even when it is the hero
