...
1. [X] Add ~org-astro-image-default-layout~ to ~ox-astro-config.el~
2. [X] Update ~org-astro--format-image-component~ to include layout prop
3. [X] Add tests: one with layout applied, one with config set to nil/none to confirm no prop (regression), and one covering render-map/remote images
4. [X] Update docs/instructions.org with the new defcustom and how to disable
5. [X] Update suggested-enhancements.org to mark as DONE
