...
* 2025-11-30 ID Map Defaults to First Destination
** Summary
- Fixed ID map construction so files without an explicit `#+DESTINATION_FOLDER`/`#+ASTRO_POSTS_FOLDER` fall back to the first `org-astro-known-posts-folders` entry. Ensures existing IDs always map to an outfile and aren’t flagged as missing.
** Files
- `ox-astro-helpers.el`
** Commit
- `f80f68d` fix: default destination for id map
