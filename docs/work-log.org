...
* 2026-07-07 Untrack scratch artifacts and remove verified dead code

** Summary
Followed up on the low-risk review items after the regex correctness fix:
untracked local scratch/large artifacts and removed verified zero-caller helper
code.

** Changes
- Added exact ignore entries in
  [[file+emacs:/Users/jay/Dropbox/github/ox-astro/.gitignore][.gitignore]]
  for root scratch exports/debug files, the large Astro docs dump, and the
  local debug JPG.
- Removed these tracked artifacts from the Git index while leaving the local
  files on disk:
  - root scratch files such as =test.org=, =example-post.org=,
    =linkpeek-test.org=, =debug.log=, and =diagnosis.md=
  - [[file+emacs:/Users/jay/Dropbox/github/ox-astro/llms-full-astro-documentation.txt][llms-full-astro-documentation.txt]]
  - [[file+emacs:/Users/jay/Dropbox/github/ox-astro/debug/assets/images/posts/QR1-0941.jpg][QR1-0941.jpg]]
- Removed zero-caller helpers from the code:
  - =org-astro--insert-keyword-at-end-of-block=
  - =org-astro--update-image-path-in-file=
  - both duplicate =org-astro--collect-raw-images-from-tree-region= definitions
  - the dead image-suggestions write-path helpers
- Kept the live raw-image wrapping path and the interactive image-suggestion
  parse/apply commands.
- Updated
  [[file+emacs:/Users/jay/Dropbox/github/ox-astro/docs/design-architecture.org][design-architecture.org]]
  and
  [[file+emacs:/Users/jay/Dropbox/github/ox-astro/docs/concept-map.org][concept-map.org]]
  so they no longer reference the deleted raw-image-region helper.

** Verification
- =rg -n "org-astro--(insert-keyword-at-end-of-block|update-image-path-in-file|collect-raw-images-from-tree-region|generate-image-paths-comment-block|insert-or-replace-suggestions-block|upsert-image-paths-comment)" *.el docs/design-architecture.org docs/concept-map.org test README.org CHANGE-LOG.org=
  returns no matches.
- Local copies of the untracked artifacts still exist and are ignored.
- =emacs --batch -Q -L . --eval "(progn (require 'ox-astro) (princ \"loaded ox-astro\n\"))"=
- =mkdir -p tmp-test && TMPDIR=/Users/jay/Dropbox/github/ox-astro/tmp-test/ emacs --batch -Q -L . -l ert -l test/conversation-export-test.el -l test/date-title-and-subtree-test.el -l test/image-pipeline-test.el -l test/theme-reorder-test.el --eval "(ert-run-tests-batch t)"; status=$?; rm -rf tmp-test; exit $status=
- Result: =30/30= tests passed.
- =git diff --check=

* 2026-07-07 Fix verified regex escaping findings from Claude Fable review

** Summary
Triaged Claude Fable's repo review against the current working tree and fixed
only the verified low-risk correctness issues.  Left the larger repo-hygiene and
image-pipeline refactor recommendations untouched because the exporter is
currently working and those changes have higher blast radius.

** Changes
- Fixed single-backslash Elisp regex metacharacters in
  [[file+emacs:/Users/jay/Dropbox/github/ox-astro/ox-astro-helpers-core.el][ox-astro-helpers-core.el]],
  [[file+emacs:/Users/jay/Dropbox/github/ox-astro/ox-astro-image-handlers.el][ox-astro-image-handlers.el]],
  and
  [[file+emacs:/Users/jay/Dropbox/github/ox-astro/ox-astro-transcode.el][ox-astro-transcode.el]].
- Moved the raw plain-text image manifest registration back inside its =let=
  binding in
  [[file+emacs:/Users/jay/Dropbox/github/ox-astro/ox-astro-image-handlers.el][ox-astro-image-handlers.el]].
- Made the full-buffer slug lookup in
  [[file+emacs:/Users/jay/Dropbox/github/ox-astro/ox-astro.el][ox-astro.el]]
  use a real =save-restriction= body.
- Removed the duplicate in-function =org-astro-debug-log-file= defcustom from
  [[file+emacs:/Users/jay/Dropbox/github/ox-astro/ox-astro.el][ox-astro.el]].
- Added regression tests in
  [[file+emacs:/Users/jay/Dropbox/github/ox-astro/test/image-pipeline-test.el][test/image-pipeline-test.el]]
  for raw plain-text image paths, generated image-suggestion parsing,
  org-roam keyword anchor insertion, and strict YouTube host matching.
- Added cleanup for two existing hero-image tests that were leaving generated
  files under
  [[file+emacs:/Users/jay/Dropbox/github/ox-astro/assets/images/posts/][assets/images/posts/]].

** Verification
- Default macOS temp location reproduced a pre-existing test-environment
  problem: image tests tried to recurse through =/var/folders/.../AppTranslocation/...=
  and failed with =Result too large= before any code changes were accepted.
- Hermetic temp-dir test run passed:
  =mkdir -p tmp-test && TMPDIR=/Users/jay/Dropbox/github/ox-astro/tmp-test/ emacs --batch -Q -L . -l ert -l test/conversation-export-test.el -l test/date-title-and-subtree-test.el -l test/image-pipeline-test.el -l test/theme-reorder-test.el --eval "(ert-run-tests-batch t)"; status=$?; rm -rf tmp-test; exit $status=
- Result: =30/30= tests passed.
- =emacs --batch -Q -L . --eval "(progn (require 'ox-astro) (princ \"loaded ox-astro\n\"))"=
- =git diff --check=

* 2026-06-23 Preserve root-relative site links in MDX

** Summary
Fixed root-relative Astro/public links like =[[/media/posts/...][label]]= so Org export preserves them as =/media/posts/...= instead of converting them to =file:///media/posts/...=.

** Root cause
Org parses links beginning with a slash as =type="file"= links, even when the author intends a site-root URL. The fallback Markdown exporter then runs those through =org-export-file-uri=, producing broken MDX links such as =file:///media/posts/how-to-build-tidy-habits/jay-30-day-tidy-sprint.md=.

** Fix
- Added =org-astro--site-root-relative-path-p= in [[file:../ox-astro-transcode.el][ox-astro-transcode.el]] to distinguish site-root URLs from common local filesystem roots.
- Added an =org-astro-link= branch that emits site-root file links directly as Markdown links before the generic =org-md-link= fallback.
- Added a regression test for the exact Socratic tidy-habits sprint download link.

** Verification
- Ran =emacs --batch -Q -L . -l ert -l test/date-title-and-subtree-test.el --eval "(ert-run-tests-batch 'org-astro-preserves-site-root-file-links)"=.

* 2026-03-27 Fix duplicate model banner for section-leading THEME/MODEL keywords

** Summary
Heading-scoped =#+THEME:= and =#+MODEL:= keywords (e.g., inside =* Claude=)
produced duplicate =<div class="model-banner">= output.  Fixed by moving the
"consumed" decision from transcode time to a parse-tree filter.

** Root cause
Org transcodes section children /before/ calling the headline transcoder.  So
the keyword handler in =ox-astro-handlers.el= emitted the banner into the
section body first, and then the headline handler in =ox-astro-transcode.el=
re-emitted it as a hoisted prefix.  The "strip from contents" fallback failed
because trailing whitespace differed between the two emission paths (=\n= vs
=\n\n=).

** Fix
- Added =org-astro--pre-mark-heading-theme-model= parse-tree filter that runs
  before transcoding and marks section-leading THEME/MODEL keywords as
  =:astro-consumed=.
- The keyword transcoder already checks =:astro-consumed= and returns ="="=,
  so it no longer emits into section contents.
- The headline transcoder remains the sole emitter via its existing
  =theme-prefix-for-heading= / =model-prefix-for-heading= functions.
- Kept the strip-from-contents logic as a safety net with a more forgiving
  regex for model banners.
- Added regression test =org-astro-empty-theme-section-does-not-duplicate-model-banner=.

** Files changed
- =ox-astro-handlers.el= — new =org-astro--pre-mark-heading-theme-model= filter
- =ox-astro-transcode.el= — removed debug instrumentation, relaxed safety-net regex
- =ox-astro.el= — registered new filter in =:filters-alist=
- =test/theme-reorder-test.el= — regression test for the exact failing pattern

** Verification
- =emacs --batch -Q -L . -l ert -l test/theme-reorder-test.el --eval "(ert-run-tests-batch 'org-astro-empty-theme-section-does-not-duplicate-model-banner)"=
- Re-exported:
  =/Users/jay/Dropbox/roam/chatgpt-outputs/20260326235900-should_vinh_move_to_singapore_or_hong_kong.org=
- Verified output:
  =/Users/jay/Dropbox/github/astro-monorepo/apps/socratic/src/content/blog/should-vinh-move-to-singapore-or-hong-kong.mdx=
- Confirmed top-of-file output now shows exactly:
  - one ={/* theme: claude */}= marker
  - one =<div class="model-banner">Claude Opus 4.6</div>=
  - one =# Claude= heading

* 2026-03-27 Remove misleading source-rewrite comments from image handlers

** Summary
Removed the misleading =Don't rewrite org file= comments from the public/GIF
branches in =ox-astro-image-handlers.el=.

** Why
- Those comments read like a general pipeline rule even though ox-astro's
  documented contract is the opposite for normal copied/downloaded images:
  export writes canonical asset paths back into the Org source.
- Leaving that wording in place made the recent regression easier to justify
  and harder to catch quickly.

** Fix
- Deleted the misleading inline comments and left the code behavior itself
  unchanged for the public/GIF branches.
- Kept the normal copied/downloaded image branches rewriting the Org source.

* 2026-03-27 Restore Org image source rewriting after copied/downloaded asset export

** Summary
Restored the intended image pipeline behavior: when ox-astro copies a local
image or downloads a remote image into the app asset tree, it also rewrites the
Org source link to that new canonical in-app path.

** Why
- ox-astro's documented contract is to keep canonical metadata and canonical
  exported asset paths in the Org source.
- A temporary regression disabled source rewriting for normal copied/remote
  images, which contradicted the docs and prior implementation history.
- That behavior was appropriate only for specific public/GIF cases, not the
  normal asset-copy flow.

** Fix
- Restored =:rewrite-path downloaded= for normal remote images.
- Restored =:rewrite-path target-path= for normal copied local images.
- Replaced the incorrect regression test with one that verifies Org source
  rewriting after export.

** Verification
- Re-exported the Vinh post and confirmed the Org image link was rewritten from
  =/Users/jay/Downloads/...= to the copied app asset path.

* 2026-03-27 Normalize canonical slugs and dedupe preamble SLUG keywords

** Summary
Fixed a slug drift regression where exported Org/MDX metadata, image folders, and
Astro routes could disagree about casing. A mixed-case =#+SLUG:= like
=should-vinh-move-to-singapore-or-hong-Kong= could leak into frontmatter and
asset paths even though the Astro app canonicalized the route to lowercase.

** Root Cause
- Slug handling was split across multiple places, and they did not all treat
  =:slug= the same way.
- Some paths trusted =:slug= verbatim, while others re-slugified from title.
- The helper that writes keywords after the org-roam preamble inserted new
  keywords but did not remove duplicate later =#+SLUG:= lines that were already
  present in the preamble.
- That meant a stale mixed-case slug could survive in the Org source and keep
  affecting later export steps.

** Fix
- Added =org-astro--normalize-slug= and =org-astro--resolve-slug= to centralize
  canonical slug resolution.
- Updated filename generation, frontmatter assembly, hero/asset folder
  selection, and image manifest processing to use the centralized canonical slug.
- Updated preflight metadata generation so existing =#+SLUG:= values are
  normalized, not just missing ones.
- Detect duplicate =#+SLUG:= keywords and force a rewrite when duplicates are
  present.
- Reworked =org-astro--upsert-keyword-after-roam= so it replaces an existing
  preamble keyword and deletes duplicates instead of blindly inserting a new one.

** Verification
- Source Org now has one lowercase =#+SLUG:= for the Vinh post.
- Export now writes
  =should-vinh-move-to-singapore-or-hong-kong.mdx= with lowercase
  =slug:= and lowercase image import paths.
- Stale mixed-case MDX and asset folder were deleted automatically on re-export.
- =npm run build= passes in =apps/socratic= and generates
  =/should-vinh-move-to-singapore-or-hong-kong/=.

* 2026-03-16 Suppress fallback hero images from body (unify with explicit hero behavior)

** Summary
Changed hero inline suppression to apply to all hero images, not just explicit
ones. Previously, fallback hero selection (first inline image auto-promoted to
frontmatter) kept the inline copy visible in the body, causing duplication when
the Astro layout also renders the frontmatter =image:= as a hero.

** Rule
Whenever =image:= exists in frontmatter, the page layout will render it as a
hero. The exporter should therefore suppress the inline copy. Suppression fires
when: hero exists AND hero is visible (not =#+HIDE_HERO_IMAGE: true=) AND the
image does not have =:repeat t=.

** Fix
- Renamed =explicit-hero= to =has-hero= in
  =org-astro--hero-inline-suppression-enabled-p= and broadened the check to
  include =:astro-hero-image= (set during fallback selection), not just explicit
  keyword properties.
- Removed =:astro-hero-explicit= from the suppression decision (it remains
  informational).
- Updated test expectations and added =:repeat t= escape-hatch test.

** Files Modified
- =ox-astro-helpers-core.el= — suppression logic
- =ox-astro-handlers.el= — body filter comment
- =test/image-pipeline-test.el= — flipped fallback test, added repeat test
- =docs/instructions.org= — updated hero suppression docs
- =docs/codebase-wisdom.org= — updated gotcha entry
- =docs/work-log.org= — this entry

** Verification
- =ox-astro-fallback-hero-suppresses-inline-image-test= — fallback hero suppressed
- =ox-astro-fallback-hero-repeat-preserves-inline-image-test= — =:repeat t= escape hatch
- =ox-astro-explicit-hero-suppresses-matching-inline-image-test= — unchanged, still passes

* 2026-03-17 Fix gallery regression for repeated hero image and hermetic snapshot fixture

** Summary
Fixed the lingering =ox-astro-gallery-export-test= failure by addressing the
real regression and the brittle fixture setup. A repeated standalone use of the
hero image in the gallery fixture was being swallowed by the final body cleanup,
and the test itself depended on image files existing in =~/Downloads=.

** Root Cause
- Hero suppression had two layers:
  1. tree/transcoder suppression via =org-astro--image-component-for-record=
  2. final body-string cleanup in =ox-astro-handlers.el=
- The tree-level pass correctly suppressed only the first hero occurrence.
- The later regex cleanup then removed the first remaining matching =<Image>=
  in the rendered body, which in the gallery fixture was an intentional later
  standalone repeat of the same hero image.
- Separately, the gallery snapshot test copied a real Org fixture containing
  =/Users/jay/Downloads/...= paths, so test reliability depended on the local
  machine having those files.

** Fix
- Made hero suppression occurrence-aware by threading the current element
  =:begin= through image rendering and suppressing only the first standalone
  hero occurrence.
- Tagged manifest occurrences with whether they came from inside a gallery so a
  gallery-origin hero does not accidentally suppress later standalone reuse.
- Kept the final body cleanup as a fallback only: it now runs only when
  suppression is enabled, a suppressible standalone occurrence exists, and the
  tree-level pass did not already mark =:astro-hero-body-suppressed=.
- Hardened the gallery export test to rewrite fixture =~/Downloads= links into a
  temp source directory and seed minimal fixture images there.
- Normalized =orgPath:= in the snapshot helper and refreshed the expected MDX.

** Files Modified
- =ox-astro-helpers-core.el= — occurrence-aware hero suppression
- =ox-astro-image-handlers.el= — track whether an occurrence came from a gallery
- =ox-astro-transcode.el= — thread occurrence begin positions through image rendering
- =ox-astro-handlers.el= — narrow regex cleanup into a guarded fallback
- =test/image-pipeline-test.el= — hermetic gallery fixture setup and normalization
- =test/gallery_test_rigorous.expected.mdx= — refreshed snapshot
- =docs/codebase-wisdom.org= — captured the non-obvious suppression/testing insight
- =docs/work-log.org= — this entry

** Verification
- =emacs --batch -Q --eval "(setq load-prefer-newer t)" -L /Users/jay/Dropbox/github/ox-astro -l /Users/jay/Dropbox/github/ox-astro/test/image-pipeline-test.el -f ert-run-tests-batch-and-exit=
- Result: 10 tests passed, 0 unexpected

* 2026-03-07 Prevent duplicate SUBHED insertion on re-export

** Summary
Hardened metadata preflight so repeated exports do not append duplicate
=#+SUBHED:= lines when excerpt keywords already exist in the parse tree.

** Root Cause
- Excerpt presence detection relied primarily on the export =info= plist.
- In some contexts, =info= can be stale/incomplete relative to the current
  parse tree, so preflight treated excerpt as missing and inserted another
  =SUBHED=.

** Fix
- In =ox-astro.el= preflight metadata section, added a tree-level fallback
  check for non-empty excerpt keywords:
  - =ASTRO_EXCERPT=
  - =EXCERPT=
  - =SUBHED=
  - =DESCRIPTION=
- Keep existing =info= checks, but require insertion only when both =info= and
  tree-level checks report excerpt missing.
- In =ox-astro-handlers.el= final output filter, added cleanup for converted
  org-roam Links output that can appear as a plain markdown link paragraph
  before the first heading (e.g., =[📄 Note](file:///...)=).

** Tests
- Added ERT regression:
  - =org-astro-reexport-does-not-duplicate-subhed=
- Added ERT regression:
  - =org-astro-strips-roam-links-source-preamble-from-body=
- Test exports the same file twice and asserts source still has exactly one
  =#+SUBHED:= line.

** Files Modified
- =ox-astro.el=
- =ox-astro-handlers.el=
- =test/date-title-and-subtree-test.el=
- =docs/work-log.org=
- =docs/codebase-wisdom.org=

** Verification
- =emacs --batch -Q -L . --eval "(progn (find-file \"ox-astro.el\") (check-parens) (kill-buffer) (find-file \"test/date-title-and-subtree-test.el\") (check-parens) (kill-buffer) (princ \"parens-ok\"))"=
- =emacs --batch -Q -L . -f batch-byte-compile ox-astro.el test/date-title-and-subtree-test.el=
- =emacs --batch -Q -L . -l ert -l test/date-title-and-subtree-test.el --eval "(ert-run-tests-batch-and-exit)"=

* 2026-02-21 Jaydocs hero AVIF rendering fix (downstream app resolver)

** Summary
Fixed missing hero/cover rendering for AVIF images in Jaydocs when frontmatter
=image:= pointed to a valid =~/assets/images/...avif= path.

** Root Cause
ox-astro correctly exported AVIF paths, but Jaydocs' image resolver omitted AVIF
from =import.meta.glob(...)= in =apps/jaydocs/src/utils/images.ts=. As a result,
=findImage()= returned =null= for hero images while inline MDX images still
rendered.

** Fix (astro-monorepo)
- Updated =apps/jaydocs/src/utils/images.ts= glob to include =avif= and =AVIF=.

** Verification
- Confirmed `/desks` renders AVIF in both places:
  - hero cover block
  - inline body image
- Confirmed OG/Twitter image tags now resolve to the AVIF source path.

* 2026-02-21 Preserve fallback hero image inline when hero is not explicit

** Summary
Fixed missing first-inline-image output when the image was auto-selected as the
frontmatter hero via fallback logic (first body image), but no explicit hero
was configured.

** Root Cause
Inline hero suppression ran for any hero match, including fallback hero
selection. In destinations that do not render a hero component, this caused the
first image to disappear from both hero and body.

** Fix
- Added =org-astro--hero-inline-suppression-enabled-p= to centralize suppression
  gating.
- Track whether hero was explicitly configured via
  =org-astro--get-cover-image= (= :astro-hero-explicit =).
- Apply suppression only when hero is explicit and visible (not hidden), while
  preserving existing =:repeat t= support.
- Updated body-filter fallback cleanup to use the same suppression gate.

** Superseded
This entry's behavior was reversed on 2026-03-16. All hero images (explicit and
fallback) are now suppressed from the body by default. See entry below.
...
- Added regression tests:
  - =ox-astro-fallback-hero-keeps-inline-image-test=
  - =ox-astro-explicit-hero-suppresses-matching-inline-image-test=

* 2026-02-21 Fix symlinked path lookup for image render records

** Summary
Fixed image lookup failures when org files reference images via symlinked paths
(e.g., =/Users/jay/Library/CloudStorage/Dropbox/...= vs =/Users/jay/Dropbox/...=).

** Root Cause
The render map stores paths as strings and uses =string-equal= for lookup.
When an org file contains a path through a symlink, the string comparison
fails against the resolved canonical path stored in the manifest.

** Fix
Added =file-truename= canonicalization in two places:
1. =org-astro--build-render-map= - Store both original and canonical paths as keys
2. =org-astro--lookup-render-record= - Try canonical path lookup as fallback

** Files Modified
- =ox-astro-helpers-core.el= (lookup canonicalization)
- =ox-astro-image-handlers.el= (render map key additions)

** Test Case
Org file with AVIF image at symlink path:
: [[/Users/jay/Library/CloudStorage/Dropbox/github/.../image.avif]]

Before fix: Image imported but =<Image>= component missing from body
After fix: Image correctly rendered (or suppressed if hero image)

* 2026-02-21 AVIF image support across export pipeline

** Summary
Added =.avif= detection across ox-astro image processing paths, including
manifest collection, paragraph/plain-text image detection, gallery extraction,
final markdown-image rewrites, and remote-download extension normalization.

** Changes
- Added AVIF extension matching to:
  - =ox-astro-image-handlers.el=
  - =ox-astro-transcode.el=
  - =ox-astro-export-helpers.el=
  - =ox-astro-table-handlers.el=
  - =ox-astro-handlers.el=
  - =update-image-paths.el=
- Added AVIF byte-signature detection fallback in
  =org-astro--detect-image-type-from-bytes= for =ftypavif/ftypavis= brands.
- Added regression tests in =test/image-pipeline-test.el=:
  - =ox-astro-avif-signature-detection-test=
  - =ox-astro-collect-raw-image-paths-includes-avif-test=
- Added a codebase wisdom note documenting extension matcher duplication and
  the need to update all matcher sites together.

** Verification
- =emacs --batch -Q -L . --eval "(setq load-prefer-newer t)" -l ert -l test/image-pipeline-test.el --eval "(ert-run-tests-batch 'ox-astro-avif-signature-detection-test)"=
- =emacs --batch -Q -L . --eval "(setq load-prefer-newer t)" -l ert -l test/image-pipeline-test.el --eval "(ert-run-tests-batch 'ox-astro-collect-raw-image-paths-includes-avif-test)"=
- =emacs --batch -Q -L . --eval "(setq load-prefer-newer t)" -l ert -l test/image-pipeline-test.el --eval "(ert-run-tests-batch 'ox-astro-hero-image-keyword-test)"=

* 2026-01-16 Related Thread keyword export (#+RELATED_THREAD)

** Summary
Added =#+RELATED_THREAD:= keyword support to export a structured =relatedThread= frontmatter object.

** Behavior
- Keyword format: =slug | optional note=
- Exported frontmatter:
  - =relatedThread: { slug, note }=
- Supports =RELATED_THREAD=, =RELATED-THREAD=, and =ASTRO_RELATED_THREAD=

** Files Modified
- =ox-astro-metadata.el=
- =ox-astro-helpers-core.el=

* 2026-01-14 Stale asset directory cleanup on slug change

** Summary
Extended stale MDX cleanup to also delete orphaned image directories when a post's slug changes.

** Problem
When renaming a post (changing its slug), =org-astro--cleanup-stale-mdx-files= deleted the old MDX file but left behind the =src/assets/images/posts/{old-slug}/= directory as orphaned cruft.

** Solution
Added guarded asset cleanup: when a stale MDX is deleted, ox-astro now:
1. Resolves the app root (via =src/content= path traversal)
2. Scans =src/= and =public/= for any references to the old slug's asset paths
3. Only deletes =src/assets/images/posts/{slug}/= if truly unreferenced

** New Functions (ox-astro-export-helpers.el)
- =org-astro--resolve-app-root-from-output= — finds app root containing =src/content=
- =org-astro--slug-assets-dir= — builds path =src/assets/images/posts/{slug}/=
- =org-astro--slug-assets-referenced-p= — scans for references in mdx, md, astro, js, ts, json, yaml, css, html, org files
- =org-astro--cleanup-stale-asset-dirs= — deletes unreferenced asset directories

** Modified Functions
- =org-astro--cleanup-stale-mdx-files= — now collects deleted slugs and calls =org-astro--cleanup-stale-asset-dirs=

** Safety Features
- Reference scanning checks both =assets/images/posts/{slug}/= and =/images/posts/{slug}/= patterns
- Short-circuits scan when first reference found
- Logs "keeping" message when assets are still referenced
...
** Verification
- =emacs --batch -Q -L /Users/jay/Dropbox/github/ox-astro -l /tmp/ox-astro-stale-cleanup-test.el=
...
* 2026-02-09 Public /og cover image support

** Summary
Allow =#+COVER_IMAGE:= values that use public web paths like =/og/...= to pass through unchanged, so Cloudflare-protected sites can serve OG images without relying on hashed =/_astro/= assets.
...
* 2026-02-12 Silence babel export noise for conversation blocks

** Summary
Suppressed verbose babel progress messages for conversation transcript blocks
during astro export (e.g., repeated =org-babel-exp process user at position ...=).

** Changes
- Added no-op babel executors:
  - =org-babel-execute:user=
  - =org-babel-execute:assistant=
- Added around advice for =org-babel-exp-src-block=:
  - Detects block language
  - Dynamically binds =noninteractive= for selected languages
  - Scoped to active astro export via =org-astro--export-in-progress=
  - Guarded with =advice-member-p= to avoid duplicate advice on reload
...
** Verification
- Batch behavior check with simulated interactive mode:
  - Outside astro export: user/assistant/python still log progress
  - Inside astro export: only python logs progress; user/assistant are silent

** Follow-up Validation (real transcript exports)
- Source files validated:
  - =/Users/jay/Dropbox/roam/chatgpt-outputs/20260212131525-claude_encouraging_me_to_text_emily_original.org=
  - =/Users/jay/Dropbox/roam/chatgpt-outputs/20260212133316-claude_encouraging_me_to_text_emily.org=
- Confirmed both source files contain =106= =#+begin_src user= blocks.
- Export check result: =0= occurrences of =org-babel-exp process ...= in both runs (no user/assistant spam).
- Output files written/updated:
  - =/Users/jay/Library/CloudStorage/Dropbox/github/astro-monorepo/apps/jaydocs/src/content/blog/claude-encouraging-me-to-text-emily-original.mdx=
  - =/Users/jay/Library/CloudStorage/Dropbox/github/astro-monorepo/apps/jaydocs/src/content/blog/claude-encouraging-me-to-text-emily.mdx=
- Note: The originally referenced non-=original= 13:15 source path was not present; validation used the existing =..._original.org= plus the 13:33 derived file.

* 2026-02-13 Deferred headline raw-value export fix

** Summary
Fixed a parse-tree filter crash caused by Org deferred headline values during
theme marker insertion.

** Changes
- Added =org-astro--headline-title-string= to resolve headline titles safely
  when =:raw-value= is deferred (vector wrapper) or absent.
- Updated =org-astro--ensure-inline-theme-markers= to use resolved headline
  titles before string operations (=downcase= / =member=).
- Hardened fuzzy-link heading resolution in =org-astro-link= to fall back from
  deferred =:raw-value= to interpreted =:title=.
- Documented the gotcha in =docs/codebase-wisdom.org=.
...
** Verification
- =emacs --batch -Q -L . --eval "(progn (find-file \"ox-astro-handlers.el\") (check-parens) (kill-buffer) (find-file \"ox-astro-transcode.el\") (check-parens) (kill-buffer) (princ \"parens-ok\"))"=
- =emacs --batch -Q -L . -f batch-byte-compile ox-astro-handlers.el ox-astro-transcode.el=

* 2026-02-16 Deferred src/example block value export fix

** Summary
Fixed an export crash when Org returned deferred vectors for src/example block
=:value= properties during MDX transcoding.

** Changes
- Added =org-astro--resolve-deferred-string= helper to resolve deferred Org
  vector wrappers into strings safely.
- Updated =org-astro-src-block= to resolve block content before regex/string
  transforms.
- Updated =org-astro-example-block= to resolve block content before trimming
  and fence rendering.
- Documented the gotcha in =docs/codebase-wisdom.org=.
...
** Verification
- =emacs --batch -Q -L . --eval "(progn (find-file \"ox-astro-helpers-core.el\") (check-parens) (kill-buffer) (find-file \"ox-astro-transcode.el\") (check-parens) (kill-buffer) (princ \"parens-ok\"))"=
- =emacs --batch -Q -L . -f batch-byte-compile ox-astro-helpers-core.el ox-astro-transcode.el=
- =emacs --batch -Q -L . --eval "(progn (require 'ox-astro-transcode) (let* ((block '(src-block (:language \"user\" :parameters nil :value [org-element-deferred identity \"one---two\"])) ) (out (org-astro-src-block block nil nil))) (princ out)))"=
- =emacs --batch -Q -L . --eval "(progn (require 'ox-astro-transcode) (let* ((block '(example-block (:switches \"fountain\" :value [org-element-deferred identity \"alpha\\n\"])) ) (out (org-astro-example-block block nil nil))) (princ out)))"=

** Follow-up Documentation
- Added a troubleshooting note to =docs/instructions.org= for
  =wrong-type-argument stringp [org-element-deferred ...]= errors and the
  expected fix path.

* 2026-02-16 Conversation block export determinism fix

** Summary
Fixed intermittent empty conversation fences in exported MDX when Babel
language defaults resolved to =:exports results= for transcript block
languages (=user=, =assistant=, etc.).

** Changes
- Added exporter-scoped macro =org-astro--with-conversation-babel-code-exports=
  to force =:exports code= for conversation-style languages during export, with
  restore-on-exit behavior via =unwind-protect=.
- Applied the macro to:
  - =org-astro-export-as-mdx=
  - both =org-export-to-file= passes in =org-astro-export-to-mdx=
- Documented the gotcha and troubleshooting flow in:
  - =docs/codebase-wisdom.org=
  - =docs/instructions.org=
...
** Verification
- Simulated hostile defaults:
  - Set =org-babel-default-header-args:user= and =...:assistant= to
    =((:exports . "results"))=
- Exported the Pasternak transcript and verified output counts:
  - =user_total=25=
  - =user_empty=0=
  - =verse_total=1=
  - =verse_empty=0=
- Re-exported real target file:
  - =/Users/jay/Dropbox/github/astro-monorepo/apps/socratic/src/content/blog/what-we-carry-with-us.mdx=
  - Verified no empty conversation fences.

* 2026-03-27 Preserve missing image links during export

** Summary
Fixed an exporter regression where standalone image paragraphs could disappear from MDX entirely when ox-astro failed to build a render record for them.

** Changes
- Updated =ox-astro-transcode.el=
  - Changed =org-astro--handle-broken-image-paragraph= to fall back to normal Markdown paragraph export when image recovery fails, instead of returning an empty string
  - Passed paragraph =contents= into the broken-image fallback helper so normal export output is preserved
- Added regression test in =test-image-rendering.el=
  - =org-astro-missing-local-image-link-is-preserved=
- Re-exported the Vinh post and confirmed both image references now remain in the generated MDX

** Verification
- =emacs --batch -Q -L . --eval "(setq load-prefer-newer t)" -l ert -l test-image-rendering.el --eval "(ert-run-tests-batch 'org-astro-missing-local-image-link-is-preserved)"=
- =emacs --batch -Q -L . --eval "(setq load-prefer-newer t)" -l ert -l test-image-rendering.el --eval "(ert-run-tests-batch 'org-astro-image-rendering-produces-render-map)"=

* 2026-02-16 Conversation export regression test and cleanup

** Summary
Followed through on hardening after the determinism fix by adding a dedicated
regression test, removing redundant global Babel defaults from handlers, and
cleaning stale local bytecode artifacts.

** Changes
- Added ERT regression test:
  - =org-astro-conversation-blocks-survive-hostile-babel-defaults=
  - Simulates hostile defaults (=:exports results=) for conversation languages
    and asserts exported fences remain non-empty.
- Removed redundant global Babel default =setq= lines from:
  - =ox-astro-handlers.el=
- Removed stale local bytecode files from repo working tree:
  - =ox-astro-handlers.elc=
  - =ox-astro-helpers-core.elc=
  - =ox-astro-transcode.elc=
- Updated docs:
  - Added wisdom/prevention note to =docs/codebase-wisdom.org=
  - Added regression test command to =docs/instructions.org=

** Files Modified
- =test/conversation-export-test.el=
- =ox-astro-handlers.el=
- =docs/codebase-wisdom.org=
- =docs/instructions.org=
- =docs/work-log.org=
...
** Verification
- =emacs --batch -Q -L . -l ert -l test/conversation-export-test.el --eval "(ert-run-tests-batch 'org-astro-conversation-blocks-survive-hostile-babel-defaults)"=
- Confirmed test passes with non-empty =user=, =assistant=, and =verse= fences.

* 2026-06-29 Remove stale transcode bytecode after empty user prompt export

** Summary
Diagnosed an interactive Org 9.8 export regression where =#+begin_src user=
blocks exported as empty =```user= fences even though the source Org blocks still
contained prompt text.

** Root Cause
The live Org 9.8 daemon was loading ignored local bytecode:
=/Users/jay/Dropbox/github/ox-astro/ox-astro-transcode.elc=.
Loading =/Users/jay/Dropbox/github/ox-astro/ox-astro-transcode.el= directly
restored non-empty conversation fences, confirming stale bytecode rather than
source content or frontmatter as the culprit.

** Changes
- Removed ignored local bytecode artifacts:
  - =/Users/jay/Dropbox/github/ox-astro/ox-astro-transcode.elc=
  - =/Users/jay/Dropbox/github/ox-astro/test/date-title-and-subtree-test.elc=
- Re-exported:
  - Source: =/Users/jay/Dropbox/roam/chatgpt-outputs/20260629151922-sundowners_voice_vibe_coding.org=
  - Output: =/Users/jay/Dropbox/github/astro-monorepo/apps/socratic/src/content/blog/sundowners-voice-vibe-coding.mdx=
- Updated =docs/codebase-wisdom.org= with the Org 9.8 stale-bytecode recurrence.

** Verification
- Fresh source resolution:
  - =emacs --batch -Q -L . --eval "(progn (require 'ox-astro) (princ (format \"src-block-file=%S org-version=%s\\n\" (symbol-file 'org-astro-src-block) (org-version))))"=
  - =emacs --batch -Q -L /Users/jay/.emacs.d/elpa/develop/org-9.8.6 -L . --eval "(progn (require 'ox-astro) (princ (format \"src-block-file=%S org-version=%s\\n\" (symbol-file 'org-astro-src-block) (org-version))))"=
- Regression tests:
  - =emacs --batch -Q -L . -l ert -l test/conversation-export-test.el --eval "(ert-run-tests-batch 'org-astro-conversation-blocks-survive-hostile-babel-defaults)"=
  - =emacs --batch -Q -L /Users/jay/.emacs.d/elpa/develop/org-9.8.6 -L . -l ert -l test/conversation-export-test.el --eval "(ert-run-tests-batch 'org-astro-conversation-blocks-survive-hostile-babel-defaults)"=
- Real export verification:
  - =user_total=15=
  - =user_empty=0=

* 2026-06-29 Add bytecode guardrails for ox-astro reload/export

** Summary
Added prevention so stale ignored =.elc= files cannot silently break
conversation-block export again.

** Changes
- Updated =/Users/jay/Dropbox/github/ox-astro/ox-astro.el=:
  - Added =org-astro--ensure-transcode-source-loaded=.
  - Export entry points now reload
    [[file+emacs:/Users/jay/Dropbox/github/ox-astro/ox-astro-transcode.el][📄 ox-astro-transcode.el]]
    from source when critical transcoders were loaded from =.elc= bytecode.
- Updated Jay's Emacs reload helper in
  [[file+emacs:/Users/jay/emacs/emacs-settings/shared-functions.org][📄 shared-functions.org]]:
  - Added =jay/ox-astro-delete-bytecode=.
  - Startup and =ox-astro-reload= now delete repo-local =.elc= files before
    loading ox-astro.
- Kept the generated local file
  [[file+emacs:/Users/jay/emacs/emacs-settings/shared-functions.el][📄 shared-functions.el]]
  in sync on disk.
- Updated [[file+emacs:/Users/jay/Dropbox/github/ox-astro/docs/instructions.org][📄 instructions.org]]
  and [[file+emacs:/Users/jay/Dropbox/github/ox-astro/docs/codebase-wisdom.org][📄 codebase-wisdom.org]].

** Verification
- Deliberately recreated
  [[file+emacs:/Users/jay/Dropbox/github/ox-astro/ox-astro-transcode.elc][📄 ox-astro-transcode.elc]]
  and confirmed Org 9.8 reloads
  [[file+emacs:/Users/jay/Dropbox/github/ox-astro/ox-astro-transcode.el][📄 ox-astro-transcode.el]]
  from source:
  - =src-block-file="/Users/jay/Dropbox/github/ox-astro/ox-astro-transcode.el"=
- Regression tests passed under both Org versions:
  - =emacs --batch -Q -L . -l ert -l test/conversation-export-test.el --eval "(ert-run-tests-batch 'org-astro-conversation-blocks-survive-hostile-babel-defaults)"=
  - =emacs --batch -Q -L /Users/jay/.emacs.d/elpa/develop/org-9.8.6 -L . -l ert -l test/conversation-export-test.el --eval "(ert-run-tests-batch 'org-astro-conversation-blocks-survive-hostile-babel-defaults)"=
- Real export still preserved all prompt bodies:
  - Source: [[file+emacs:/Users/jay/Dropbox/roam/chatgpt-outputs/20260629151922-sundowners_voice_vibe_coding.org][📄 20260629151922-sundowners_voice_vibe_coding.org]]
  - Output: [[file+emacs:/Users/jay/Dropbox/github/astro-monorepo/apps/socratic/src/content/blog/sundowners-voice-vibe-coding.mdx][📄 sundowners-voice-vibe-coding.mdx]]
  - =user_total=15=
  - =user_empty=0=
- Byte-compiling =/Users/jay/Dropbox/github/ox-astro/ox-astro.el= succeeded
  with pre-existing warnings about obsolete/free Org variables, but no hard
  failure.
- Removed generated =.elc= files after verification.

* 2026-06-29 Trim conversation block line-ending whitespace

** Summary
Fixed an exporter hygiene issue where copied chat prompt whitespace could survive
inside generated Markdown fences and trip =git diff --check=.

** Changes
- Updated [[file+emacs:/Users/jay/Dropbox/github/ox-astro/ox-astro-transcode.el][📄 ox-astro-transcode.el]]:
  - Added a helper that trims trailing whitespace from each source-block line.
  - Applied it only to =user=, =prompt=, =quote=, =poetry=, and =verse= blocks.
  - Left paragraph soft-break generation untouched because two trailing spaces
    are intentional Markdown syntax there.
- Updated [[file+emacs:/Users/jay/Dropbox/github/ox-astro/test/conversation-export-test.el][📄 conversation-export-test.el]]
  with a regression test covering block trimming and preserved paragraph soft
  breaks.
- Updated [[file+emacs:/Users/jay/Dropbox/github/ox-astro/docs/codebase-wisdom.org][📄 codebase-wisdom.org]]
  with the scoped-whitespace lesson.

** Verification
- =emacs --batch -Q -L . -l ert -l test/conversation-export-test.el --eval "(ert-run-tests-batch 'org-astro-conversation-blocks-trim-line-ending-whitespace)"=
- =emacs --batch -Q -L . -l ert -l test/conversation-export-test.el --eval "(ert-run-tests-batch t)"=
- =emacs --batch -Q -L /Users/jay/.emacs.d/elpa/develop/org-9.8.6 -L . -l ert -l test/conversation-export-test.el --eval "(ert-run-tests-batch t)"=
