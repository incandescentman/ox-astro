...
** Contract: Export writes canonical metadata and canonical copied asset paths back into Org
- *Date:* 2026-03-27
- *Mechanism:* ox-astro does not just generate MDX. It also updates the source
  Org file with canonical export state like =#+SLUG:= and rewritten paths for
  normal copied/downloaded assets.
- *Actionable insight:* If a fix proposal preserves original image paths in Org
  to avoid some downstream cleanup bug, that is probably fighting the pipeline.
  Fix the cleanup or path-resolution bug without changing the source-rewrite
  contract.

** Gotcha: Root-relative site links parse as Org file links
- *Date:* 2026-06-23
- *Trigger:* An Org link like =[[/media/posts/file.md][Download]]= exported to
  MDX as =[Download](file:///media/posts/file.md)=.
- *Mechanism:* Org parses a slash-leading link as =type="file"= with
  =path="/media/posts/file.md"=. If =org-astro-link= lets that fall through to
  =org-md-link=, Org treats it as a local absolute filesystem path and prefixes
  =file://=.
- *Actionable insight:* Keep site-root URL handling inside =org-astro-link= and
  run it before the generic Markdown fallback. Use
  =org-astro--site-root-relative-path-p= for slash-leading Astro/public links
  that should stay root-relative in MDX.

** Trap: Org transcodes section children before calling the headline transcoder
- *Date:* 2026-03-27
- *Mechanism:* In Org export, a headline's section contents (keywords,
  paragraphs, etc.) are transcoded into strings /before/ the headline
  transcoder receives them as =contents=.  This means a keyword transcoder
  cannot rely on the headline transcoder having set =:astro-consumed= yet.
- *Actionable insight:* If you need to suppress a keyword inside a heading
  section (to hoist it as a heading prefix), do the suppression in a
  parse-tree filter that runs before transcoding — not in the headline
  transcoder.  The current solution is =org-astro--pre-mark-heading-theme-model=.
  Never use "emit then strip from contents" as the primary defense; it is
  fragile because whitespace in the rendered string may differ between
  emission paths.
...
** Behavior: Export rewrites image paths in the Org source
- *Date:* 2026-03-27
- *Mechanism:* When ox-astro copies an image from its original location (e.g.
  =~/Downloads/=) into =src/assets/images/posts/{slug}/=, it also rewrites the
  =[[...]]= link in the Org source buffer to point to the new assets path. This
  happens via =org-astro--rewrite-org-image-path= → =org-astro--update-source-buffer-image-path=
  at ox-astro-image-handlers.el:551. The rewrite only fires when =:rewrite-path=
  is non-nil in the materialize result.
- *Actionable insight:* If image paths in an Org file still point to Downloads or
  another external location after export, don't manually fix them — diagnose why
  the rewrite didn't fire. Common causes: =:rewrite-path= set to nil, source
  buffer not open during export, or the image hitting the "already inside assets"
  branch (which skips rewriting since the path is already correct).

** Gotcha: Raw `#+SLUG` values can drift from the app's canonical route
- *Date:* 2026-03-27
- *Trigger:* A post exported with =#+SLUG: should-vinh-move-to-singapore-or-hong-Kong=
  while the Astro app canonicalized the actual route to
  =/should-vinh-move-to-singapore-or-hong-kong/=.
- *Mechanism:* Some exporter paths trusted =:slug= verbatim, while others
  slugified from title. Duplicate =#+SLUG:= lines in the Org preamble made the
  problem worse because a stale later slug could survive even after a new
  normalized slug was inserted near the top.
- *The Fix:* Centralize slug resolution in =org-astro--resolve-slug=, normalize
  existing slug keywords with =org-astro--normalize-slug=, and make
  =org-astro--upsert-keyword-after-roam= replace/remove duplicate preamble
  keywords instead of only inserting.
- *Actionable insight:* Treat slug normalization and slug deduplication as the
  same problem. If a slug bug appears in routes, also inspect the Org preamble
  for duplicate =#+SLUG:= lines before debugging Astro.
...
** Gotcha: Image extension support is duplicated across modules
- *Date:* 2026-02-21
- *Trigger:* Adding a new format (like =.avif=) in only one matcher path caused inconsistent behavior between pre-processing, transcoding, galleries, and final output rewrites.
- *Mechanism:* Extension regexes are intentionally duplicated in multiple files (manifest collection, transcoders, table/gallery handling, and final output filter).
- *Actionable insight:* When adding image formats, update all extension matchers together and add regression tests for both path matching and byte-signature normalization.

** Gotcha: Symlinked paths break image render record lookups
- *Date:* 2026-02-21
- *Trigger:* AVIF image was imported but =<Image>= component missing from body, even though record lookup succeeded for JPGs in same file.
- *Mechanism:* Org file referenced image via symlink path (=/Users/jay/Library/CloudStorage/Dropbox/...=), but manifest stored canonical path (=/Users/jay/Dropbox/...=). String comparison failed.
- *Fix:* Added =file-truename= canonicalization in two places:
  1. =org-astro--build-render-map= - Store both original and canonical paths as hash keys
  2. =org-astro--lookup-render-record= - Try canonical path lookup as fallback
- *Prevention:*
  - Always use =file-truename= before storing paths in hash tables
  - When comparing paths, canonicalize both sides OR store multiple key variants
  - macOS CloudStorage symlinks are a common source of path mismatches
- *Actionable insight:* Path comparison bugs often manifest as "it works for some images but not others" - check if affected paths go through different symlink routes

** Hero images are always suppressed from the body (explicit and fallback)
- *Date:* 2026-03-16 (updated from 2026-02-21)
- *Rule:* Whenever an image is promoted to frontmatter =image:=, the inline copy is suppressed from the body to avoid duplication with the layout-rendered hero.
- *Mechanism:* =org-astro--hero-inline-suppression-enabled-p= checks for any hero (=:astro-hero-image= or explicit keywords), not just explicit ones. Both explicit (=#+HERO_IMAGE:=) and fallback (first inline image) heroes are suppressed.
- *Escape hatch:* Use =#+ATTR_ORG: :repeat t= before an image to force it to appear inline even when it is the hero.
- *Actionable insight:* If a user sees a missing first image, check whether =:repeat t= is needed, or whether =#+HIDE_HERO_IMAGE: true= was set (which disables suppression so the inline image stays).

** Gotcha: Hero suppression must target a specific occurrence, not just a matching image variable
- *Date:* 2026-03-17
- *Trigger:* A post reused the same image first as a gallery/hero source and later as a standalone body image; the later standalone image disappeared.
- *Mechanism:* We had two suppression passes. Tree-level rendering correctly suppressed the first hero duplicate, but a later regex cleanup removed the first matching =<Image src={heroVar} ...>= still present in the rendered string. If the original hero occurrence was inside a gallery, that "first remaining match" could actually be a later intentional repeat.
- *Fix:* Track occurrence metadata (including =:begin= and whether the occurrence came from inside a gallery) and suppress only the first standalone hero occurrence. Keep the string-level cleanup only as a fallback when tree-level suppression never fired.
- *Actionable insight:* When the same image file can appear multiple times, suppression/removal logic should key off parse-tree occurrence identity, not path equality or "first regex match" in the final output.

** Gotcha: Broken-image paragraph fallback could silently delete image links
- *Date:* 2026-03-27
- *Trigger:* Standalone Org image paragraphs vanished from exported MDX when their render record was missing, even though the source links were still present in the Org file.
- *Mechanism:* =org-astro-paragraph= routed any paragraph whose interpreted text looked image-like through =org-astro--handle-broken-image-paragraph=. That helper returned an empty string when no render record was found, so valid image-link paragraphs were dropped instead of falling back to normal Markdown export.
- *Fix:* When broken-image recovery fails, fall back to =org-md-paragraph= (with soft-break preservation) instead of returning =\"\"=.
- *Actionable insight:* Recovery code for malformed image paragraphs must never be lossy. If repair fails, preserve the original content rather than deleting it.

** Gotcha: Heading-level THEME/MODEL emission must be decided before transcoding
- *Date:* 2026-03-27
- *Trigger:* A section like =* Claude= followed by =#+THEME:= and =#+MODEL:=
  emitted duplicate model banners in MDX.
- *Mechanism:* Org transcodes section children before it calls the headline
  transcoder. If heading-scoped metadata is only marked as consumed during
  headline transcoding, the keyword transcoder can already have emitted the
  model banner into section contents; the headline path then hoists and emits
  it again.
- *Fix:* Pre-mark section-leading =#+THEME:= / =#+MODEL:= keywords as
  =:astro-consumed= in a parse-tree filter, then let the headline transcoder be
  the sole emitter for hoisted theme/model prefixes.
- *Actionable insight:* For heading-scoped metadata, prefer deciding ownership
  in parse-tree preprocessing rather than trying to "emit then strip" during
  later transcoding stages.

** Gotcha: Downstream app image resolver can silently drop AVIF heroes
- *Date:* 2026-02-21
- *Trigger:* Frontmatter image path was AVIF and valid, but hero/cover section rendered empty while inline image still appeared.
- *Mechanism:* App-level =findImage()= resolver used =import.meta.glob= without AVIF extensions, so frontmatter lookups returned =null=.
- *Fix:* Add =avif= (and uppercase =AVIF=) to the resolver glob in the consuming app (Jaydocs: =apps/jaydocs/src/utils/images.ts=).
- *Actionable insight:* Exporter format support and app resolver format support must stay in sync; verify both pipelines when adding formats.
...
** Gotcha: Excerpt presence checks based only on INFO can duplicate SUBHED
- *Date:* 2026-03-07
- *Trigger:* Re-export appends additional =#+SUBHED:= even though excerpt keywords already exist.
- *Mechanism:* Metadata preflight checked excerpt presence via export =info= plist. In some contexts, =info= may be stale/incomplete relative to the parsed tree.
- *Fix:* Add tree-level fallback check for non-empty excerpt keywords (=ASTRO_EXCERPT=, =EXCERPT=, =SUBHED=, =DESCRIPTION=) before inserting =SUBHED=.
- *Actionable insight:* Presence checks that gate source mutation should verify the current parse tree, not only cached export plist values.

** Gotcha: org-roam Links preamble can survive as plain markdown link line
- *Date:* 2026-03-07
- *Trigger:* Exported MDX contains a top-of-body line like =[📄 Note](file:///...)= even after existing Links/Source cleanup.
- *Mechanism:* Depending on Org markdown rendering, =- Links ::= definition list can collapse to a plain markdown link paragraph without literal =Links:= text, bypassing older regexes.
- *Fix:* In final output filter, strip file-link-only lines in the pre-heading prefix (frontmatter/import area) to catch this converted form safely.
- *Actionable insight:* Cleanup patterns should account for semantic output variants, not just one textual rendering of the same source structure.
...
** Gotcha: Conversation source blocks should not preserve accidental line-end spaces
- *Date:* 2026-06-29
- *Trigger:* =git diff --check= reports trailing whitespace inside generated =```user= / =```prompt= / =```quote= fences after exporting copied chat transcripts.
- *Mechanism:* Paragraphs intentionally use two trailing spaces for Markdown line breaks, but conversation-style source block lines do not need line-ending spaces. Whole-block trimming removes only final block whitespace, so interior source lines can leak copied prompt whitespace into MDX.
- *Fix:* Trim trailing whitespace per line inside =org-astro-src-block= only for =user=, =prompt=, =quote=, =poetry=, and =verse= blocks.
- *Actionable insight:* Do not add a global final-output trailing-space cleanup. It would erase intentional paragraph soft breaks; keep whitespace normalization scoped to block types where line-ending spaces are export noise.
...
- *2026-06-29 recurrence:* Empty =```user= fences in
  =/Users/jay/Dropbox/github/astro-monorepo/apps/socratic/src/content/blog/sundowners-voice-vibe-coding.mdx=
  reproduced only in Jay's Org 9.8 interactive/daemon path. Direct calls to
  =org-astro-src-block= preserved =:value=, but full export wrote empty
  fences until stale ignored bytecode
  =/Users/jay/Dropbox/github/ox-astro/ox-astro-transcode.elc= was removed or
  =ox-astro-transcode.el= was loaded directly.
- *2026-06-29 prevention:* =org-astro-export-as-mdx= and
  =org-astro-export-to-mdx= now call =org-astro--ensure-transcode-source-loaded=
  before export, and Jay's Emacs =ox-astro-reload= deletes repo-local =.elc=
  files before reloading.
...
- *Prevention:*
  ...
  - If behavior is inconsistent, check runtime source with =symbol-file= and verify whether stale =.elc= is being used; deleted ignored bytecode is usually safer than letting Emacs prefer it.
...
** Gotcha: Elisp regex metacharacters require double escaping inside strings
- *Date:* 2026-07-07
- *Trigger:* A review found patterns like ="\(Links\|Source\)"=,
  ="\\.(png\\|jpe?g\\|webp)"=, and ="youtu\.be"= in
...
- *Mechanism:* The Elisp reader consumes a single backslash before many
  characters.  By the time the regex engine sees the pattern, ="\("= has become
  a literal =(=, and ="\\.(png\\|webp)"= behaves like =\\.png= OR =webp=
  instead of an anchored extension group.
- *Fix:* Use doubled regex escapes inside strings, e.g. ="\\(Links\\|Source\\)"=,
  ="\\.\\(png\\|jpe?g\\|webp\\)"=, and ="youtu\\.be"=.
- *Prevention:*
  - For any new image/path/keyword regex, add or update an ERT case that proves
    the intended sample string matches.
  - Prefer shared extension constants like =org-astro--media-extension-regexp=
    where possible.
  - When in doubt, run a tiny =string-match-p= repro in =emacs --batch= before
    trusting visual regex inspection.
