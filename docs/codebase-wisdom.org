...
- *Mechanism:* Standard Markdown collapses single newlines to spaces. ox-astro's `org-astro-plain-text` adds two trailing spaces to each non-empty line (except the last), creating Markdown "soft breaks" that render as `<br>` in HTML.
- *Example:*
  ```
...
- *Implementation:* `ox-astro-transcode.el` - `org-astro-plain-text` function
