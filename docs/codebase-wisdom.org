...
** Gotcha: Fallback hero suppression can hide the first body image
- *Date:* 2026-02-21
- *Trigger:* First image disappeared from article body even though image import/frontmatter looked correct.
- *Mechanism:* Hero suppression treated fallback hero selection (first inline image) the same as explicit hero images, so the inline image was removed even on sites that do not render a hero.
- *Fix:* Added =:astro-hero-explicit= tracking in hero selection and gated suppression logic on explicit hero usage (plus existing =:repeat t= and hide-hero checks).
- *Actionable insight:* Inline suppression should be driven by rendering intent, not just “this image was chosen for frontmatter.”
