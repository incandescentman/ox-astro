...
*** Stale MDX Cleanup
*Goal*: Automatically delete old MDX files when slug changes

*Flow*:
1. After export completes → 2. Scan output directory for =.mdx= files → 3. Read =orgPath= from each file's frontmatter → 4. Delete files matching source but with different filename
...
*Files*: =ox-astro-helpers.el= (lines 2150-2192), =ox-astro.el= (cleanup call after export)
