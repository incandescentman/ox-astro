...
- *Key features*:
  - Stale MDX cleanup: Auto-deletes old files when slug changes (uses =orgPath= in frontmatter)
  - Line break preservation: Single newlines become Markdown soft breaks (two trailing spaces)
- *Last major update*: 2026-01-14 (Line break fix - paragraph level)
...
- =org-astro--ensure-inline-theme-markers= — Inject THEME keywords for Claude/ChatGPT headings
- =org-astro--pre-mark-heading-theme-model= — Mark section-leading THEME/MODEL keywords as consumed so the keyword transcoder skips them (prevents duplicate banners)
...
*** Stale MDX + Asset Cleanup
*Goal*: Automatically delete old MDX files (and their unused asset folders) when slug changes

*Flow*:
1. After export completes → 2. Scan output directory for =.mdx= files → 3. Read =orgPath= from each file's frontmatter → 4. Delete files matching source but with different filename → 5. For each deleted slug, scan the app for references and delete =src/assets/images/posts/{slug}/= if unreferenced

*How it works*:
- Each exported MDX contains =orgPath: /path/to/source.org= in frontmatter
- On export, =org-astro--cleanup-stale-mdx-files= compares this against the source file
- Files from the same source with different slugs are deleted
- Asset cleanup resolves the app root (by locating =src/content=) and scans =src/= and =public/= for =assets/images/posts/{slug}/= or =/images/posts/{slug}/= references before deleting directories

*Key functions*:
- =org-astro--extract-orgpath-from-mdx= — Reads orgPath from MDX frontmatter
- =org-astro--cleanup-stale-mdx-files= — Scans directory and deletes stale files
- =org-astro--cleanup-stale-asset-dirs= — Deletes slug asset directories if unreferenced

*Files*: =ox-astro-export-helpers.el=, =ox-astro.el= (cleanup call after export)
...
- **Single newlines** → Markdown soft breaks (two trailing spaces + newline)

*** Line Break Preservation
In standard Markdown, single newlines within a paragraph are collapsed to spaces. ox-astro preserves single newlines as visible line breaks by adding two trailing spaces before each newline (Markdown soft break syntax).

*Why*: AI conversation transcripts often contain meaningful line breaks that would be lost with standard Markdown behavior. For example:
#+begin_example
A conclusion you can draw more cleanly, with a Bayesian lens.
Neither assistant said "Bayes" explicitly...
#+end_example

Without soft breaks, this renders as one continuous line. With soft breaks, each line renders separately.

*Implementation*: =org-astro--add-soft-breaks= helper called from =org-astro-paragraph= in =ox-astro-transcode.el=. This must happen at the paragraph level (not =org-astro-plain-text=) because plain-text only sees text fragments between other elements, not whole paragraphs.
...
Phase 1: Parse tree processing
  ...
  - org-astro--build-image-manifest
