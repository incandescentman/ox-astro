...
- *Last major update*: 2026-01-08 (Line break preservation)
...
*Implementation*: =org-astro-plain-text= in =ox-astro-transcode.el= adds two trailing spaces to each non-empty line (except the last) before joining with newlines.
